...
class Residual_block(nn.Module):
    ...
    def forward(self, input_data):
        ...
        #print(f"[ResidualBlock] res norm: {res.norm().item():.6f}")
        #print(f"[ResidualBlock] skip norm: {skip.norm().item():.6f}")
        #print(f"[ResidualBlock] x norm: {x.norm().item():.6f}")
...
class Residual_group(nn.Module):
    ...
    def forward(self, input_data):
        ...
        for n in range(self.num_res_layers):
            ...
            #print(f"[ResidualGroup] n: {n}, norm: {h.norm().item():.6f}")
            skip += skip_n  
